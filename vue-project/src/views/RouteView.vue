        ...
        <tbody>
          ...
          <template v-else>
            <tr v-for="route in filteredRoutes" :key="route.id">
              <td class="text-bold">{{ route.name }}</td>
...
const filteredRoutes = computed(() => {
  ...
  return routes.value.filter((route) =>
    [route.name, route.description, route.order].some((value) =>
...
const resetForm = () => {
  form.name = ''
  form.description = ''
  form.order = routes.value.length + 1
}
...
const saveRoute = async () => {
  ...
  try {
    ...
    if (editingRoute.value) {
      await axios.put(`/api/routes/${editingRoute.value.id}`, payload)
    } else {
